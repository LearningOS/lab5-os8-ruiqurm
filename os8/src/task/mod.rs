...
pub use process::DeadLockDetector;
...
pub fn exit_current_and_run_next(exit_code: i32) {
    ...
    //     Arc::clone(process_inner.detector.as_ref().unwrap()).mutex_dealloc(tid);
    //     Arc::clone(process_inner.detector.as_ref().unwrap()).sem_dealloc(tid);
    // }
    // drop(process_inner);
    // Record exit code
